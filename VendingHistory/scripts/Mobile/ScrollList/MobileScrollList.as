package Mobile.ScrollList
{
   ...
   public class MobileScrollList extends MovieClip
   {
      ...
      protected function setPosition() : void
      {
         ...
         var _loc1_:Number = this._direction == HORIZONTAL ? this._scrollList.width : this._scrollList.height;
         var _loc2_:Number = this._direction == HORIZONTAL ? this._bounds.width : this._bounds.height;
         var _loc3_:Number = this._direction == HORIZONTAL ? this._scrollList.x : this._scrollList.y;
         if(isNaN(this.position))
         {
            if(this.selectedIndex == -1)
            {
               ...
               {
                  this._scrollList.x = this.endListAlign ? _loc2_ - _loc1_ : 0;
               }
               else
               {
                  this._scrollList.y = this.endListAlign ? _loc2_ - _loc1_ : 0;
               }
               this.setDataOnVisibleRenderers();
               return;
            }
            _loc4_ = this._direction == HORIZONTAL ? this.selectedRenderer.x : this.selectedRenderer.y;
            if(this.canScroll)
            {
            ...
            {
               this._position = this.endListAlign ? _loc2_ - _loc1_ : 0;
            }
         }
      ...
      protected function enterFrameHandler(param1:Event) : void
      {
         ...
         if(this._bounds != null && this.canScroll)
         {
            ...
            this._velocity *= _loc2_;
            _loc3_ = this._direction == HORIZONTAL ? this._scrollList.width : this._scrollList.height;
            _loc4_ = this._direction == HORIZONTAL ? this._bounds.width : this._bounds.height;
            _loc5_ = this._direction == HORIZONTAL ? this._scrollList.x : this._scrollList.y;
            if(!this._mouseDown)
            {
               _loc6_ = 0;
            ...
            if(!isNaN(param1.stageX) && !isNaN(param1.stageY))
            {
               ...
               if(this._direction == HORIZONTAL)
               {
                  ...
                  if(this._scrollList.x >= this._bounds.x || this._scrollList.x <= this._bounds.x - (this._scrollList.width - this._bounds.width))
                  {
                     if(this.elasticity)
                     {
                        this._scrollList.x += _loc3_ * this.RESISTANCE_OUT_BOUNDS;
                     }
                  ...
                  this._velocity += (_loc2_.x - this._prevMouseDownPoint.x) * this.VELOCITY_MOVE_FACTOR;
               }
               else
               {
                  ...
                  if(this._scrollList.y >= this._bounds.y || this._scrollList.y <= this._bounds.y - (this._scrollList.height - this._bounds.height))
                  {
                     if(this.elasticity)
                     {
                        this._scrollList.y += _loc3_ * this.RESISTANCE_OUT_BOUNDS;
                     }
                  ...
                  this._velocity += (_loc2_.y - this._prevMouseDownPoint.y) * this.VELOCITY_MOVE_FACTOR;
               }
